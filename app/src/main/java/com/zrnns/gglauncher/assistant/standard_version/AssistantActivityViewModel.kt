...
class AssistantActivityViewModel(val context: Context): androidx.lifecycle.ViewModel() {
    ...
    class AssistantResultContent(val text: String, val firstImageUrl: String?, val otherImageUrls: List<String>) {
    ...
    val messageTextSize: MutableLiveData<Float> =
        MutableLiveData()
    val assistantResultContent: MutableLiveData<AssistantResultContent?> =
        MutableLiveData()
    ...
    fun requestStartAction() {
        ...
        messageColor.value = context.resources.getColor(R.color.colorPrimaryFont, context.theme) .toColor()
        messageTextSize.value = 42f
        assistantResultContent.value = null
    }
    ...
    fun assistantDisplayOutAction(resultContent: AssistantResultContent?) {
        ...
        assistantResultContent.value = resultContent
    }
        ...
        mEmbeddedAssistant = EmbeddedAssistant.Builder()
            ...
            .setConversationCallback(object : EmbeddedAssistant.ConversationCallback() {
                ...
                override fun onAssistantDisplayOut(html: String?) {
                    mMainHandler!!.post {
                        html?.let {
                            val content = AssistantResultContent(GoogleAssistantJsonParser.parse(it))
                            assistantDisplayOutAction(content)
                        } ?: run {
